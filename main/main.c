...
void app_main(void)
{
    ...
    io_conf.pin_bit_mask = (1ULL << CONFIG_LED_RED_IO) | (1ULL << CONFIG_LED_GREEN_IO);
    ...
    for (size_t i = 0; i < 5; i++)
    {
        ...
        vTaskDelay(M2T(1000));
        ESP_LOGI(TAG, "waiting pmw3901 power ON %d", 5 - i);
    ...
    {
        vTaskDelay(M2T(1000));
        gpio_set_level(CONFIG_LED_GREEN_IO, trigger);
        if (trigger)
        {
            trigger = FALSE;
        }
